...
def generate_question(difficulty):
    ...
    if difficulty == "Easy":
        ...
        if choice == 1: # solve for distance
            question = f"If an object is launched horizontally off of a {d_y:.2f} m cliff with an initial velocity of {v_x:.2f} m/s, how far away does it land? (m)"
            answer = d_x
            unit = "Horizontal Distance (m)"
        elif choice == 2: # solve for height
            question = f"If an object is launched horizontally off of a cliff at {v_x:.2f} m/s, and lands {d_x:.2f} m away, what was the height of the cliff? (m)"
            answer = d_y
            unit = "Cliff Height (m)"
        elif choice == 3:
            question = f"If an object is launched horizontally off of a {d_y:.2f} m cliff, and lands {d_x:.2f} m away, how fast was it launched? (m/s)"
            answer = v_x
            unit = "Initial Velocity (m/s)"
    elif difficulty == "Hard":
        choice = random.randint(1,3)
        if choice == 1: # solve for initial velocity and height
            question = f"If an object was launched horizontally off of a cliff and lands at {v_r:.2f} m/s at a {theta:.2f} degree angle, how fast was it launched, and from how high?"
            answer = v_x
            unit = "Initial Velocity (m/s)"
            answer2 = d_y
            unit2 = "Cliff Height (m)"
        elif choice == 2: # solve for final velocity and angle
            question = f"If an object is launched horizontally off of a cliff at {v_x:.2f} m/s, and lands {d_x:.2f} m away, what speed and angle does it land with?"
            answer = v_r
            unit = "m/s"
            answer2 = theta
            unit2 = "degrees"
        elif choice == 3: # give only height and angle
            question = f"An object is launched horizontally off of a {d_y:.2f} m cliff, and lands at a {theta:.2f} degree angle, with what speed did it land, and how far from the base of the cliff?"
            answer = v_r
